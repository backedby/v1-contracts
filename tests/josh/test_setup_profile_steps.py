...
def test_ProfileSetupHelper(je_org, ProfileSetupHelper):
    ...
    pf = ProfileSetupHelper.deploy(je_org.profiles, je_org.tiers, je_org.subscriptionsFactory, defaultPrices, defaultCids, defaultCur, defaultMul, helpers.by(je_org.deployer))
    ...
    assert je_org.tiers.getTierPrice(spTx1.return_value[0], spTx1.return_value[1], 3, defaultCur[1]) == 20e18
    assert je_org.profiles.getProfile(spTx1.return_value[0])[0] == je_org.anons[0].address

    spTx2 = pf.AdvancedProfileSetup("profile1", [1,5], ['z', 'x'], 2, helpers.by(je_org.anons[1]))
    ...
    assert je_org.tiers.getTierPrice(spTx2.return_value[0], spTx2.return_value[1], 1, defaultCur[1]) == 5e18
